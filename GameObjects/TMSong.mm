...
{
    std::map<TMSongDifficulty, ObjCPtr<TMSongSavedScore> > _scoreForDifficultyMap;
}
...
- (void)reloadScores
{
    _scoreForDifficultyMap.clear();

    for ( int i = 0; i < kNumSongDifficulties; ++i )
    {
        if ( m_nAvailableDifficultyLevels[i] > 0 )
        {
            NSString *sql = [NSString stringWithFormat:@"WHERE hash = '%@' AND difficulty = '%@'", m_sHash, [NSNumber numberWithInt:i]];
            TMSongSavedScore *val = (TMSongSavedScore *) [TMSongSavedScore findFirstByCriteria:sql];

            if ( val )
            {
                _scoreForDifficultyMap.insert(std::make_pair((TMSongDifficulty) i,
                        ObjCPtr<TMSongSavedScore>(val)));
            }
        }
    }
}
...
{
    std::map<TMSongDifficulty, ObjCPtr<TMSongSavedScore> >::iterator it = _scoreForDifficultyMap.find(difficulty);
    if ( it != _scoreForDifficultyMap.end() )
    {
